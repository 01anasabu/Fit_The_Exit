#!/usr/bin/env python3
"""
Main entry point for the Website Analyzer application.
"""
...
CHUNK_SIZE = 2  # Adjust based on your needs
MAX_WORKERS = 3  # Limit concurrent workers to avoid rate limits
...
def aggregate_with_llm(chunks_results, website_type, specific_issues, chunk_count):
    ...
    print("Using LLM to aggregate results from multiple chunks...")
...
def analyze_website_session_parallel(website_type: str, specific_issues: List[str], sessions_json: List[Dict[str, Any]]):
    """
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"🧠 Aggregating results across all {len(all_chunk_results)} successful chunks using LLM")
        print(f"{'='*60}")
        ...
        print(f"\n{'='*60}")
        print("🔄 Running: Final Orchestration on LLM-aggregated results")
        print(f"{'='*60}\n")
...
- **Issues Attempted**: {', '.join(specific_issues)}
...
def analyze_website_session(website_type: str, specific_issues: List[str], sessions_json: List[Dict[str, Any]]):
    ...
    return analyze_website_session_parallel(website_type, specific_issues, sessions_json)
...
if __name__ == "__main__":
    ...
    result, markdown_report = analyze_website_session_parallel(website_type, specific_issues, sessions_json)
